  ...
  /*  jQuery('body').on('focusin focusout', '.read', function(event){  //функция срабатывает при потере фокуса и передаёт данные функция делегированная
    ...
    $( "body" ).on('click', '.saves', function(){ //save of file
        ...
        $.ajax({
            url: path,
            type: "POST",
            data:{update:'text', text:$(this).parent().find('.read').html(), hidden:$(this).parent().find('.read').attr("data-id")},
            success: function(result){
                alert('сохранено');
...
//начало скачивания zip архива
    ...
    $("#text_ser").on("change paste keyup", function() { //chenge input
    ...
    if ($_GET['ser']){
        echo '
        $("#text_ser").val(localStorage.getItem(\'mysearch\'));
        ';
    }
    ?>
    $( ".plus" ).click(function() {
    ...
    $( "body" ).on('change', '.dow_file_into', function(){ //update file
        if(rangob!=undefined){//если у нас создавался диапазон выделения
            var formData = new FormData(); //create objecte
            var ins = this.files.length;
            for (var x = 0; x < ins; x++) {
            ...
            formData.append("id", idread);//add file into  objecte formdata
            $.ajax({
                url: "upper.php", // Url to which the request is send
                type: "POST",             // Type of request to be send, called as method
                data: formData, // Data sent to server, a set of key/value pairs (i.e. form fields and values)
                ...
                success: function(data)   // A function to be called if request succeeds
                {
                    my_select.paste(data);
                }
            });
            console.log($('[data-id='+idread+']'));
            console.log($('[data-id='+idread+']').html());
            console.log(idread);
            /* $.ajax({
             url: path,
             type: "POST",
             data:{update:'1', text:$(this).parent().find('.read').html(), hidden:$(this).parent().find('.read').attr("data-id")},
             success: function(result){
             alert('сохранено');
             today_date.html(result);
             }});*/
    ...
    /*$( ".dow_file_into" ).change(function() { //update file
    ...
    //drag end drop begin
    var dropbox; //бъектная переменная
    //dropbox = document.getElementById("dropbox");//область на которую можно переносить файлы
    //$('ul').on('click', 'a', fn);
    //начало регистрируем начало обхода элементов drop
    dropbox = document.querySelectorAll(".read");//область на которую можно переносить файлы
    [].forEach.call(dropbox, function(dropbox) {
        dropbox.addEventListener("dragenter", dragenter, false);
        dropbox.addEventListener("dragover", dragover, false);
        dropbox.addEventListener("drop", drop, false); //куда кидаем файлы .parent().find('.read')
    });
    //конец регистрируем начало обхода элементов drop
    //$('body').on('dragenter', '.read', dragenter);
    //$('body').on('dragover', '.read', dragover);
    //$('body').on('drop', '.read', drop);
    //функции обработки событий
    function dragenter(e) {
        e.stopPropagation();
        e.preventDefault();
        dropreadfile=$(this);
        data_id_for_inser=$(this).attr("data-id");
        dropreadfile.css({'background':'linear-gradient(to bottom, #1e5799 0%,#2989d8 50%,#207cca 57%)'});
        //dropreadfile.style.background = "linear-gradient(to bottom, #1e5799 0%,#2989d8 50%,#207cca 57%)"; //background: linear-gradient(to bottom, #1e5799 0%,#2989d8 50%,#207cca 57%);
        //console.log(data_id_for_inser);
    ...
    function drop(e) {
       ...
       this.parentNode.querySelector('.upper').files = e.dataTransfer.files;//находим в записи элемент с классом upper
        ...
        var files = dt.files;
        handleFiles(files); //регистрируем функцию в которой будет происходить обработка
        dropreadfile.css({'background':'white'});
    }
    function handleFiles(files) {
        $('.delete_drop').remove();//удаление предидущих элементов
        $.ajax({
            url: path,
            type: "POST",
            data:{drag_and_drop: 'file/'+data_id_for_inser},
            success: function(result){
                //$('body').append(result);//вставляем скрипт для скачивания zip архива
                //intoobj.parent().parent().next().html(result);
                ///alert(result);
                dropreadfile.parent().append('добавлены новые файлы, перезагрузите страницу');//.parent().css({'bacground':'red'});//find('.create_fol').html(result);
            }});
        for (var i = 0; i < files.length; i++) {
            var file = files[i];
            //$('.line_create').append('<br/><span class="delete_drop">'+files[i].name+'</span>');
        }
        $('.see').text('выбрано файлов '+files.length);
    }
    //drag end drop end
        ...
        if(i==''){
           alert('Введите колличество строк');
            ...
            if(j==''){
                ...
                string=string+'</table><span class="export_exel" >exel</span><span  class="nameuin"><span data-val="'+name_table+'">название</span></span><span class="t_csv">Загрузить csv</span></div><br/>';//окончание записи
    ...
    $( ".createhref" ).click(function() {
        ...
        console.log(j);
        var str_href='<a class="read_href" contenteditable="false" target="_blank" href="'+j+'">'+i+'</a>';
...
$(function(){
    ...
    //matchtable('ii');
    <?
    //bagin script for read
    function printjavascript($conn, $sql){
        if (isset($_GET['id'])){
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    //echo '<script data-id="'.$row['id'].'" class="readscript">'.$row['javascript'].'</script>';
                    echo $row['javascript'];
                }
            }
        }
    ...
    printjavascript($conn, $sql);//go script of post
    //end script for read
    ?>
